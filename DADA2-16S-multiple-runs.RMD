...
out <- filterAndTrim(fnF1, filtF1, fnR1, filtR1, 
              ...
              truncLen=c(260,200), ##adjust these lengths based your examination of the quality plots above
...
Dereplicate your reads: collapse reads that encode identical sequences to reduce downstream computational times. 
```{r}
tic()
...
Run the next code chunk for a second sequencing run. If R crashes, you can always read seqtab1.Rds back in  to your workspace.
...
These settings should ideally be the same as above. However, it is more important that you select the appropriate parameters for your data.
```{r}
tic()
...
# Third data set (and on)

Third verse, same as the first. If you only have two samples, don't run this! If you have more than three, copy and paste this code chunk as many times as necessary, making sure to increment the numbers where indicated (3 -> 4, etc). If you do not update all of the numbering, you will either get errors, or your data will be wrong and you won't know it. 

```{r}
tic()
path <- "/path/to/data/folder/03" ##CHANGE THIS to the folder with your next set of raw data

fnF3 <- sort(list.files(path, pattern="_R1_001.fastq.gz", full.names = TRUE)) ##1x number to increment (fnF3 -> fnF4 if you're on data from a fourth run)
fnR3 <- sort(list.files(path, pattern="_R2_001.fastq.gz", full.names = TRUE)) ##1X number to increment
sample.names3 <- sapply(strsplit(basename(fnF3), "_"), `[`, 1) #2x numbers to increment
filtF3 <- file.path(path, "filtered", paste0(sample.names3, "_F_filt.fastq.gz")) #2x numbers to increment
filtR3 <- file.path(path, "filtered", paste0(sample.names3, "_R_filt.fastq.gz")) #2x numbers to increment 
names(filtF3) <- sample.names3  #2x numbers to increment
names(filtR3) <- sample.names3  #2x numbers to increment
plotQualityProfile(fnF3[1:4]) #1x numbers to increment
plotQualityProfile(fnR3[1:4]) #1x numbers to increment
toc()
```
...
```{r}
tic()
out3 <- filterAndTrim(fnF3, filtF3, fnR3, filtR3, trimLeft=c(19,20),truncLen=c(240,200), maxN=0, maxEE=c(2,2), truncQ=2, minLen=75, rm.phix=TRUE, compress=TRUE, multithread=TRUE) #4x numbers to increment, adjust parameters as needed
head(out3) #1x numbers to increment
write.csv(out3,file="qc_check3.csv") #2x numbers to increment
plotQualityProfile(filtF3[1:4]) #1x numbers to increment
plotQualityProfile(filtR3[1:4]) #1x numbers to increment
toc()
```
...
```{r}
tic()
derepF3 <- derepFastq(filtF3, verbose=TRUE)  #2x numbers to increment
derepR3 <- derepFastq(filtR3, verbose=TRUE)  #2x numbers to increment

errF3 <- learnErrors(derepF3, multithread=TRUE) #2x numbers to increment
errR3 <- learnErrors(derepR3, multithread=TRUE) #2x numbers to increment

dadaF3 <- dada(derepF3, err=errF3, multithread=TRUE) #3x numbers to increment
dadaR3 <- dada(derepR3, err=errR3, multithread=TRUE) #3x numbers to increment

merger3 <- mergePairs(dadaF3, derepF3, dadaR3, derepR3, verbose=TRUE)  #5x numbers to increment
head(merger3[[1]]) #1x numbers to increment 
seqtab3 <- makeSequenceTable(merger3) #2x numbers to increment
saveRDS(seqtab3, "seqtab3.Rds") #2x numbers to increment
toc()
```

## End of data set 3

Here is a template for the three code chunks where the number has been replaced with "xxx." If you copy this into a text editor you can find/replace xxx with the number of your current data set, leaving the one below to use for next time. Don't run the xxx chunks.
```{r}
tic()
...
```{r}
seqtab <- mergeSequenceTables(seqtab1, seqtab2, seqtab3, etc) ##add all of the sequence tables you generated
...
Note that using the boost PCR protocol appears to dramatically increase the number of chimeric sequences. As per Yin, it should always be a last resort for PCR, and used only after the normal 25 and 30 cycle PCRs have failed to produce enough DNA. It's better than nothing, but results in a lot more junk. It makes the largest total amount of DNA, but if the other cycles work they will make more USABLE DNA. 
